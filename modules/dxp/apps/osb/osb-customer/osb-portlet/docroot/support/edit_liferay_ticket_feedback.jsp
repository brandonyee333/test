...
<div class="callout-content">
	...
	<div class="aui-helper-hidden" id="<portlet:namespace />commentSection">
		<div class="indent">
			...
			<input class="aui-button-input" onClick="<portlet:namespace />updateTicketFeedback();" type="button" value="<liferay-ui:message key="submit" />" />
		</div>
	</div>
</div>
...
<aui:script>
	...
	function <portlet:namespace />updateTicketFeedback() {
		...
		submitForm(document.<portlet:namespace />fm1, '<%= updateTicketFeedbackURL.toString() %>');
	}
				...
				'<portlet:actionURL name="updateTicketFeedbackSatisfied" />',
				{
					...
					on: {
						success: function(event, id, obj) {
							...
							document.<portlet:namespace />fm1.<portlet:namespace />satisfied.value = response.satisfied;
							document.<portlet:namespace />fm1.<portlet:namespace />ticketFeedbackId.value = response.ticketFeedbackId;
						}
					}
				}
			);
