	...
	<aui:form action="<%= updateAccountProjectURL.toString() %>" method="post" name="fm" onSubmit="submitForm(this); return false;">
		...
		%>
			...
			<div class="clearfix">
				...
				<div class="content-column customer-info w80">
					<aui:input label="" maxlength="<%= OSBConstants.TEXTAREA_MAX_LENGTH %>" name='<%= "field--" + fieldId + "--" %>' onKeyDown="Liferay.Util.checkTab(this); Liferay.Util.disableEsc();" title="" type="textarea" value='<%= (accountProject != null) ? accountProject.getData(fieldId) : "" %>' />
				</div>
			</div>
