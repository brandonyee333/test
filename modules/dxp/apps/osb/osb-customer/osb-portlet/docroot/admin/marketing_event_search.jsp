				...
				<div>
					<select class="multiple-select" id="<portlet:namespace /><%= displayTerms.TYPES %>" multiple name="<portlet:namespace /><%= displayTerms.TYPES %>">
						...
						%>
							...
							<option <%= ArrayUtil.contains(displayTerms.getTypes(), type) ? "selected" : "" %> value="<%= type %>"><%= LanguageUtil.get(pageContext, MarketingEventConstants.getTypeLabel(type)) %></option>
				...
				<div>
					<select class="multiple-select" id="<portlet:namespace /><%= displayTerms.GLOBAL_REGIONS %>" multiple name="<portlet:namespace /><%= displayTerms.GLOBAL_REGIONS %>">
						...
						%>
							...
							<option <%= ArrayUtil.contains(displayTerms.getGlobalRegions(), globalRegion) ? "selected" : "" %> value="<%= globalRegion %>"><%= LanguageUtil.get(pageContext, MarketingEventConstants.getGlobalRegionLabel(globalRegion)) %></option>
		...
		<aui:field-wrapper cssClass="float-container start-date" label="start-date">
			...
			<span class='<%= displayTerms.isAnytime() ? "aui-helper-hidden" : "" %>' id="<portlet:namespace />datesOptions">
				<liferay-ui:input-date
					...
					yearRangeEnd="<%= displayTerms.getStartDateLTYear() + 1 %>"
					yearRangeStart="<%= displayTerms.getStartDateGTYear() - 10 %>"
