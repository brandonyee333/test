create index IX_20B2A5A9 on OSB_AccountAttachment (accountEntryId);
create index IX_F1A6D304 on OSB_AccountAttachment (accountEntryId, accountProjectId);
create index IX_1B961F96 on OSB_AccountAttachment (accountEntryId, accountProjectId, fileName, type_);
...
create index IX_A2455F2A on OSB_AccountCustomer (userId);
...
create unique index IX_32214056 on OSB_AccountEntry (code_);
...
create index IX_7EFCE72B on OSB_AccountEntry (name);
create index IX_2FB47967 on OSB_AccountEntry (name, code_, redirectAccountEntryId);
create index IX_1DCDCBE5 on OSB_AccountEntry (partnerEntryId);
create index IX_C2D8D61C on OSB_AccountEntry (redirectAccountEntryId);
...
create index IX_EE986C81 on OSB_AccountEnvironment (accountEntryId);
create index IX_CC4721B3 on OSB_AccountEnvironment (accountEntryId, productEntryId);
create index IX_4A401F32 on OSB_AccountEnvironment (accountEntryId, productEntryId, name);

create index IX_20765BA5 on OSB_AccountEnvironmentAttachment (accountEnvironmentId);
create index IX_8CAAC4C0 on OSB_AccountEnvironmentAttachment (accountEnvironmentId, fileName);
create index IX_23139A90 on OSB_AccountEnvironmentAttachment (accountEnvironmentId, type_);

create index IX_3B5CFE75 on OSB_AccountInformation (accountEntryId, accountProjectId);
...
create index IX_ABFA544 on OSB_AccountWorker (accountEntryId);
...
create index IX_EECD6DCA on OSB_AccountWorker (userId);
...
create index IX_35580258 on OSB_AppFlag (appVersionId);
create index IX_A49D947D on OSB_AppFlag (appVersionId, type_);
create index IX_1671D402 on OSB_AppFlag (uuid_);

create index IX_477468B2 on OSB_AppPackage (appVersionId);
...
create index IX_ED5891F3 on OSB_AppPackagePlugin (appPackageId);
...
create index IX_45C8A653 on OSB_AppPricingItem (appPricingId);
...
create index IX_85BA9F9E on OSB_AppVersion (appEntryId);
...
create index IX_49DF404D on OSB_AssetAttachment (classNameId, classPK);
...
create index IX_80CBCEAD on OSB_AssetLicense (classNameId, classPK);
create index IX_519B1093 on OSB_AssetLicense (classNameId, classPK, status);
create index IX_B13824AD on OSB_AssetLicense (classNameId, classPK, usageType, licenseType);
create index IX_D674436A on OSB_AssetLicense (classNameId, classPK, usageType, licenseType, licenseTypeAllotment);
...
create index IX_339F9D26 on OSB_AssetList (assetCategoryId);
...
create index IX_5ED7CEC4 on OSB_AssetListAssetEntry (assetListId);
...
create index IX_259369D5 on OSB_AssetReceiptLicense (assetReceiptId);
create index IX_E6EEB68E on OSB_AssetReceiptLicense (assetReceiptId, startDate, endDate);
...
create index IX_98252929 on OSB_AssetReceiptLicense (ownerClassNameId, ownerClassPK, productId);
...
create index IX_512EDDE3 on OSB_AssetReceiptSupport (assetReceiptId);
create index IX_F1BA813E on OSB_AssetReceiptSupport (assetReceiptId, productClassNameId, productClassPK);
...
create index IX_C4415F67 on OSB_AssetStatsDay (classNameId, classPK);
create unique index IX_D2F824F6 on OSB_AssetStatsDay (classNameId, classPK, day, year);

create index IX_757AC64B on OSB_AssetStatsMonth (classNameId, classPK);
create unique index IX_C8BB39B6 on OSB_AssetStatsMonth (classNameId, classPK, month, year);

create index IX_2431D55F on OSB_AssetStatsWeek (classNameId, classPK);
...
create index IX_1A167D81 on OSB_AuditAction (modifiedDate);
...
create index IX_80F71C3B on OSB_CorpProjectMessage (corpProjectId);
...
create index IX_93DD5506 on OSB_CountryAppPricing (appVersionId);
...
create unique index IX_64FD3078 on OSB_CurrencyEntry (currencyCode);
...
create index IX_92A4B529 on OSB_DeveloperEntry (userId);
create index IX_59DA448C on OSB_DeveloperEntry (userId, type_);
create index IX_AB013772 on OSB_DeveloperEntry (userId, type_, status);

create index IX_AC757523 on OSB_ExternalIdMapper (classNameId, classPK);
create index IX_86A7752 on OSB_ExternalIdMapper (classNameId, classPK, type_);
create index IX_8FBE42CF on OSB_ExternalIdMapper (classNameId, type_, externalId);

create index IX_AB919B7 on OSB_HolidayCalendarRel (holidayCalendarId);
...
create index IX_244070A on OSB_LicenseEntry (productEntryId);
create index IX_8B5D7FE4 on OSB_LicenseEntry (productEntryId, portalVersionMin);
create unique index IX_7576438B on OSB_LicenseEntry (productEntryId, type_);
create index IX_12886FD on OSB_LicenseEntry (type_);
...
create index IX_F44DE07C on OSB_LicenseKey (assetReceiptLicenseId, productId, serverId, active_);
create index IX_3B9F5AEA on OSB_LicenseKey (licenseKeySetId);
create index IX_E09031D2 on OSB_LicenseKey (offeringEntryId);
create index IX_2D8DAA13 on OSB_LicenseKey (offeringEntryId, clusterId);
...
create index IX_8472FBAC on OSB_LicenseKey (offeringEntryId, licenseEntryType, complimentary, active_);
create index IX_80A769F8 on OSB_LicenseKey (orderEntryId);
create index IX_4C3397AB on OSB_LicenseKey (orderEntryId, owner);
create index IX_DE309610 on OSB_LicenseKey (productEntryName, serverId, active_);
create index IX_AFB6E8E7 on OSB_LicenseKey (productId, serverId);
create index IX_787239BF on OSB_LicenseKey (userId);
create index IX_A7B2FB73 on OSB_LicenseKey (userId, accountEntryId);
create index IX_FED7DC75 on OSB_LicenseKey (userId, assetReceiptLicenseId, productId);

create index IX_AA77CE1D on OSB_LicenseKeySet (accountEntryId);
create index IX_62FEABD6 on OSB_LicenseKeySet (userId, accountEntryId, name);

create index IX_EED5EFA6 on OSB_MarketingEvent (globalRegion);
create index IX_F81473A on OSB_MarketingEvent (type_);

create unique index IX_DC1ECDCE on OSB_OfferingBundle (name);
...
create index IX_BA1F0592 on OSB_OfferingDefinition (productEntryId);
create index IX_28E3AE89 on OSB_OfferingDefinition (productEntryId, supportResponseId);
create index IX_CFDC51BF on OSB_OfferingDefinition (productEntryId, supportResponseId, productDescription, licenses, unlimitedLicenses, supportTickets);
...
create index IX_9903B2E9 on OSB_OrderEntry (uuid_);
...
create unique index IX_2D2C67B1 on OSB_PartnerEntry (code_);
create index IX_E567A9ED on OSB_PartnerEntry (dossieraAccountKey);
create index IX_5665A174 on OSB_PartnerEntry (parentPartnerEntryId);

create index IX_C67C02A4 on OSB_PartnerWorker (partnerEntryId);
...
create index IX_53AC2A65 on OSB_PartnerWorker (userId);
...
create unique index IX_190AFA9 on OSB_ProductEntry (name);

create index IX_889263E1 on OSB_SearchFilter (userId);
...
create unique index IX_9A2345B4 on OSB_SupportResponse (name);
create index IX_21D8E69E on OSB_SupportResponse (supportLevel);

create unique index IX_CB537ED0 on OSB_SupportTeam (name);
...
create index IX_6760DF6C on OSB_SupportWorker (userId);
...
create index IX_26149242 on OSB_TicketAttachment (ticketEntryId, fileName, visibility, status);
...
create index IX_8415F49F on OSB_TicketAttachment (type_);
...
create index IX_EF2EACB9 on OSB_TicketComment (ticketEntryId);
...
create index IX_1C71ADAB on OSB_TicketComment (userId, ticketEntryId, visibility, status);
create index IX_565F15CA on OSB_TicketComment (userId, ticketEntryId, visibility, status, type_);

create index IX_CE03A0FF on OSB_TicketEntry (accountEntryId);
...
create index IX_6FF2D5CC on OSB_TicketEntry (offeringEntryId);
create index IX_A5B233EC on OSB_TicketEntry (offeringEntryId, resolution);

create index IX_5BB30859 on OSB_TicketFeedback (ticketEntryId);
create index IX_61173523 on OSB_TicketFeedback (ticketEntryId, subject);
...
create index IX_A43C0D2 on OSB_TicketFlag (ticketEntryId);
create index IX_21EF78C3 on OSB_TicketFlag (ticketEntryId, type_);
...
create index IX_539CB0C on OSB_TicketInformation (ticketEntryId);
create index IX_C15F4C19 on OSB_TicketInformation (ticketEntryId, fieldId);

create index IX_22E27E04 on OSB_TicketLink (ticketEntryId);
...
create index IX_CFAE8A40 on OSB_TicketWorker (ticketEntryId);
create index IX_26AF5491 on OSB_TicketWorker (ticketEntryId, primary_);
create index IX_C09CD9CB on OSB_TicketWorker (userId);
...
create index IX_B8FB20D4 on OSB_TrainingCustomer (classNameId, classPK);
create index IX_6CC54BBA on OSB_TrainingCustomer (classNameId, classPK, status);
create index IX_6BD1C0F9 on OSB_TrainingCustomer (userId);
create index IX_27854F5 on OSB_TrainingCustomer (userId, classNameId);
...
create index IX_192EB969 on OSB_TrainingEvent (trainingCourseId);
...
create index IX_F855F759 on OSB_TrainingWorker (userId);
