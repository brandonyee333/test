 ...
 * @author Ryan Park
 */
public class AssetStatsWeekMessageListener extends BaseMessageListener {
	...
	protected void updateAssetStatsWeek(
	...
	protected void updateAssetStatsWeeks() throws Exception {
		...
		for (int i = 0; i <= pages; i++) {
			...
			for (AppEntry appEntry : appEntries) {
				...
				updatePreviousAssetStatsWeek(
					className, classPK, startCalendar, endCalendar);

				updateAssetStatsWeek(
					className, classPK, startCalendar, endCalendar);
			}
		}
	}

	protected void updatePreviousAssetStatsWeek(
			String className, long classPK, Calendar startCalendar,
			Calendar endCalendar)
		throws Exception {
		...
		previousStartCalendar.add(Calendar.DAY_OF_MONTH, -7);

		Calendar previousEndCalendar = (Calendar)endCalendar.clone();

		previousEndCalendar.add(Calendar.DAY_OF_MONTH, -7);

		AssetStatsWeek previousAssetStatsWeek =
			AssetStatsWeekLocalServiceUtil.fetchAssetStatsWeek(
				className, classPK,
				previousEndCalendar.get(Calendar.WEEK_OF_YEAR),
				previousEndCalendar.get(Calendar.YEAR));

		if (previousAssetStatsWeek == null) {
			return;
		}
