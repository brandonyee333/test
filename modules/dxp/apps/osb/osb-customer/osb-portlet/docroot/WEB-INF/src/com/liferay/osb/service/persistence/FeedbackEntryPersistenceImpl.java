...
import com.liferay.osb.NoSuchFeedbackEntryException;
...
public class FeedbackEntryPersistenceImpl extends BasePersistenceImpl<FeedbackEntry>
	...
	public void cacheResult(FeedbackEntry feedbackEntry) {
		EntityCacheUtil.putResult(FeedbackEntryModelImpl.ENTITY_CACHE_ENABLED,
	...
	public void clearCache() {
		if (_HIBERNATE_CACHE_USE_SECOND_LEVEL_CACHE) {
			CacheRegistryUtil.clear(FeedbackEntryImpl.class.getName());
		}

		EntityCacheUtil.clearCache(FeedbackEntryImpl.class.getName());
	...
	public void clearCache(FeedbackEntry feedbackEntry) {
		EntityCacheUtil.removeResult(FeedbackEntryModelImpl.ENTITY_CACHE_ENABLED,
			FeedbackEntryImpl.class, feedbackEntry.getPrimaryKey());
	...
	public FeedbackEntry remove(long feedbackEntryId)
		...
		return remove(Long.valueOf(feedbackEntryId));
	}
	...
	protected FeedbackEntry removeImpl(FeedbackEntry feedbackEntry)
		...
		try {
			...
			BatchSessionUtil.delete(session, feedbackEntry);
		}
		catch (Exception e) {
			throw processException(e);
		}
	...
	@Override
	public FeedbackEntry updateImpl(
		...
		try {
			...
			BatchSessionUtil.update(session, feedbackEntry, merge);
	...
	/**
	 * Returns the feedback entry with the primary key or throws a {@link com.liferay.portal.NoSuchModelException} if it could not be found.
	 *
	 * @param primaryKey the primary key of the feedback entry
	 * @return the feedback entry
	 * @throws com.liferay.portal.NoSuchModelException if a feedback entry with the primary key could not be found
	 * @throws SystemException if a system exception occurred
	 */
	@Override
	...
	public FeedbackEntry findByPrimaryKey(long feedbackEntryId)
		throws NoSuchFeedbackEntryException, SystemException {
		FeedbackEntry feedbackEntry = fetchByPrimaryKey(feedbackEntryId);

		if (feedbackEntry == null) {
			if (_log.isWarnEnabled()) {
				_log.warn(_NO_SUCH_ENTITY_WITH_PRIMARY_KEY + feedbackEntryId);
			}
	...
	public FeedbackEntry fetchByPrimaryKey(long feedbackEntryId)
		throws SystemException {
		FeedbackEntry feedbackEntry = (FeedbackEntry)EntityCacheUtil.getResult(FeedbackEntryModelImpl.ENTITY_CACHE_ENABLED,
				FeedbackEntryImpl.class, feedbackEntryId);

		if (feedbackEntry == _nullFeedbackEntry) {
			return null;
		}
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public List<FeedbackEntry> findAll() throws SystemException {
	...
	public List<FeedbackEntry> findAll(int start, int end,
		...
		List<FeedbackEntry> list = (List<FeedbackEntry>)FinderCacheUtil.getResult(finderPath,
				finderArgs, this);
	...
	public void afterPropertiesSet() {
		String[] listenerClassNames = StringUtil.split(GetterUtil.getString(
					com.liferay.util.service.ServiceProps.get(
						"value.object.listener.com.liferay.osb.model.FeedbackEntry")));

		if (listenerClassNames.length > 0) {
			try {
				List<ModelListener<FeedbackEntry>> listenersList = new ArrayList<ModelListener<FeedbackEntry>>();

				for (String listenerClassName : listenerClassNames) {
					Class<?> clazz = getClass();

					listenersList.add((ModelListener<FeedbackEntry>)InstanceFactory.newInstance(
	...
	public void destroy() {
		EntityCacheUtil.removeCache(FeedbackEntryImpl.class.getName());
	...
	private static Log _log = LogFactoryUtil.getLog(FeedbackEntryPersistenceImpl.class);
	private static FeedbackEntry _nullFeedbackEntry = new FeedbackEntryImpl() {
			@Override
			public Object clone() {
				return this;
			}
			...
			public CacheModel<FeedbackEntry> toCacheModel() {
				return _nullFeedbackEntryCacheModel;
			}
		};

	private static CacheModel<FeedbackEntry> _nullFeedbackEntryCacheModel = new CacheModel<FeedbackEntry>() {
			public FeedbackEntry toEntityModel() {
				return _nullFeedbackEntry;
			}
		};
}