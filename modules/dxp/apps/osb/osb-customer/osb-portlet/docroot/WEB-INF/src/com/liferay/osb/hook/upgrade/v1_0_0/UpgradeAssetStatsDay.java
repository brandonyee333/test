 ...
 * @author Peter Shin
 */
public class UpgradeAssetStatsDay extends UpgradeProcess {
	...
	protected void updateAssetStatsDays() throws Exception {
		if (tableHasColumn("OSB_AssetStatsDay", "startDate")) {
			runSQL("alter table OSB_AssetStatsDay drop column startDate");
		}

		if (tableHasColumn("OSB_AssetStatsDay", "endDate")) {
			runSQL("alter table OSB_AssetStatsDay drop column endDate");
		}
		...
		int count = AssetStatsDayLocalServiceUtil.getAssetStatsDaiesCount();
		...
		for (int i = 0; i <= pages; i++) {
			...
			List<AssetStatsDay> assetStatsDays =
				AssetStatsDayLocalServiceUtil.getAssetStatsDaies(start, end);

			for (AssetStatsDay assetStatsDay : assetStatsDays) {
				AssetStatsDayLocalServiceUtil.deleteAssetStatsDay(
					assetStatsDay);
			}
		}
		...
		for (int i = 0; i <= pages; i++) {
			...
			for (AppEntry appEntry : appEntries) {
				...
				updateAssetStatsDays(className, classPK, calendar, now);
			}
		}
	}
