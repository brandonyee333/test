...
import com.liferay.osb.NoSuchAccountProjectException;
...
public class AccountProjectPersistenceImpl extends BasePersistenceImpl<AccountProject>
	...
	public void cacheResult(AccountProject accountProject) {
		EntityCacheUtil.putResult(AccountProjectModelImpl.ENTITY_CACHE_ENABLED,
	...
	public void clearCache() {
		if (_HIBERNATE_CACHE_USE_SECOND_LEVEL_CACHE) {
			CacheRegistryUtil.clear(AccountProjectImpl.class.getName());
		}

		EntityCacheUtil.clearCache(AccountProjectImpl.class.getName());
	...
	public void clearCache(AccountProject accountProject) {
		EntityCacheUtil.removeResult(AccountProjectModelImpl.ENTITY_CACHE_ENABLED,
			AccountProjectImpl.class, accountProject.getPrimaryKey());
	...
	public AccountProject remove(long accountProjectId)
		throws NoSuchAccountProjectException, SystemException {
		return remove(Long.valueOf(accountProjectId));
	}
	...
	protected AccountProject removeImpl(AccountProject accountProject)
		...
		try {
			...
			BatchSessionUtil.delete(session, accountProject);
		}
		catch (Exception e) {
			throw processException(e);
		}
	...
	@Override
	public AccountProject updateImpl(
		...
		try {
			...
			BatchSessionUtil.update(session, accountProject, merge);
		...
		if (isNew || !AccountProjectModelImpl.COLUMN_BITMASK_ENABLED) {
			FinderCacheUtil.clearCache(FINDER_CLASS_NAME_LIST_WITHOUT_PAGINATION);
		}
			...
			if ((accountProjectModelImpl.getColumnBitmask() &
				...
				Object[] args = new Object[] {
						Long.valueOf(accountProjectModelImpl.getOriginalAccountEntryId())
					};
				...
				args = new Object[] {
						Long.valueOf(accountProjectModelImpl.getAccountEntryId())
					};
	...
	/**
	 * Returns the account project with the primary key or throws a {@link com.liferay.portal.NoSuchModelException} if it could not be found.
	 *
	 * @param primaryKey the primary key of the account project
	 * @return the account project
	 * @throws com.liferay.portal.NoSuchModelException if a account project with the primary key could not be found
	 * @throws SystemException if a system exception occurred
	 */
	@Override
	...
	public AccountProject findByPrimaryKey(long accountProjectId)
		throws NoSuchAccountProjectException, SystemException {
		AccountProject accountProject = fetchByPrimaryKey(accountProjectId);

		if (accountProject == null) {
			if (_log.isWarnEnabled()) {
				_log.warn(_NO_SUCH_ENTITY_WITH_PRIMARY_KEY + accountProjectId);
			}
	...
	public AccountProject fetchByPrimaryKey(long accountProjectId)
		throws SystemException {
		AccountProject accountProject = (AccountProject)EntityCacheUtil.getResult(AccountProjectModelImpl.ENTITY_CACHE_ENABLED,
				AccountProjectImpl.class, accountProjectId);

		if (accountProject == _nullAccountProject) {
			return null;
		}
	...
	public List<AccountProject> findByAccountEntryId(long accountEntryId,
		...
		List<AccountProject> list = (List<AccountProject>)FinderCacheUtil.getResult(finderPath,
				finderArgs, this);
		...
		if (list == null) {
			...
			try {
				...
				list = (List<AccountProject>)QueryUtil.list(q, getDialect(),
						start, end);
			}
			catch (Exception e) {
				throw processException(e);
	 ...
	 * @throws SystemException if a system exception occurred
	 */
	public List<AccountProject> findAll() throws SystemException {
	...
	public void afterPropertiesSet() {
		String[] listenerClassNames = StringUtil.split(GetterUtil.getString(
					com.liferay.util.service.ServiceProps.get(
						"value.object.listener.com.liferay.osb.model.AccountProject")));

		if (listenerClassNames.length > 0) {
			try {
				List<ModelListener<AccountProject>> listenersList = new ArrayList<ModelListener<AccountProject>>();

				for (String listenerClassName : listenerClassNames) {
					Class<?> clazz = getClass();

					listenersList.add((ModelListener<AccountProject>)InstanceFactory.newInstance(
	...
	public void destroy() {
		EntityCacheUtil.removeCache(AccountProjectImpl.class.getName());
	...
	private static Log _log = LogFactoryUtil.getLog(AccountProjectPersistenceImpl.class);
	private static AccountProject _nullAccountProject = new AccountProjectImpl() {
			@Override
			public Object clone() {
				return this;
			}
			...
			public CacheModel<AccountProject> toCacheModel() {
				return _nullAccountProjectCacheModel;
			}
		};

	private static CacheModel<AccountProject> _nullAccountProjectCacheModel = new CacheModel<AccountProject>() {
			public AccountProject toEntityModel() {
				return _nullAccountProject;
			}
		};
}