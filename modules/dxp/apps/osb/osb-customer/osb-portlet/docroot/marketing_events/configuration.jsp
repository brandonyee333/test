...
String editorContent = PrefsParamUtil.getString(portletPreferences, request, editorParam, ContentUtil.get(PortletPropsValues.MARKETING_EVENTS_PORTLET_HEADER_CONTENT));
%>
			...
			<c:otherwise>
				<aui:select label="language" name="languageId" onChange='<%= renderResponse.getNamespace() + "updateLanguage(this);" %>'>
					...
					for (int i = 0; i < locales.length; i++) {
						...
						if (Validator.isNotNull(portletPreferences.getValue("headerContent_" + LocaleUtil.toLanguageId(locales[i]), StringPool.BLANK))) {
