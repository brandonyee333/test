 ...
 * @author Amos Fong
 */
@Component(immediate = true, service = ZendeskCategoryWebService.class)
public class DefaultZendeskCategoryWebService
	implements ZendeskCategoryWebService {

	public SearchHits<ZendeskCategory> getZendeskCategories(Query query)
		throws PortalException {

		JSONObject responseJSONObject = _zendeskBaseWebService.get(
			ZendeskRESTEndpoints.URL_API_V2 + "help_center/categories.json",
			query.getParameters());

		return toSearchHits(responseJSONObject);
	}

	protected SearchHits<ZendeskCategory> toSearchHits(
			JSONObject responseJSONObject)
		throws PortalException {

		SearchHits<ZendeskCategory> searchHits = new SearchHitsImpl<>();
		...
		searchHits.setResults(zendeskConverter.toZendeskCategories(jsonArray));
