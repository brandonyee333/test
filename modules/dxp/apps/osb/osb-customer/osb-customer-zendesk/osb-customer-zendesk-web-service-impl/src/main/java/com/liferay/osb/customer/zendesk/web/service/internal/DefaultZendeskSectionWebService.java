 ...
 * @author Amos Fong
 */
@Component(immediate = true, service = ZendeskSectionWebService.class)
public class DefaultZendeskSectionWebService
	implements ZendeskSectionWebService {
	...
	public SearchHits<ZendeskSection> getZendeskSections(Query query)
		...
		JSONObject responseJSONObject = _zendeskBaseWebService.get(
			ZendeskRESTEndpoints.URL_API_V2 + "help_center/sections.json",
			query.getParameters());

		return toSearchHits(responseJSONObject);
	}

	protected SearchHits<ZendeskSection> toSearchHits(
			JSONObject responseJSONObject)
		throws PortalException {

		SearchHits<ZendeskSection> searchHits = new SearchHitsImpl<>();
		...
		JSONArray jsonArray = responseJSONObject.getJSONArray("sections");

		searchHits.setResults(zendeskConverter.toZendeskSections(jsonArray));
