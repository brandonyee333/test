...
@Component(
	immediate = true,
	service = SynchronizeDXPCloudStatusPageMessageListener.class
)
public class SynchronizeDXPCloudStatusPageMessageListener
	...
	protected void doReceive(Message message) throws Exception {
		Map<String, String> subscribers =
			_dxpCloudStatusPageSubscriptionUtil.getSubscribers();

		for (Map.Entry<String, String> subscriber : subscribers.entrySet()) {
			User user = _userLocalService.fetchUserByEmailAddress(
				OSBCustomerConstants.COMPANY_ID, subscriber.getKey());
			...
			if (!osbDXPCloudSubscription) {
				expandoBridge.setAttribute(
					"osbDXPCloudSubscription", true, false);
			}
		}
	}
