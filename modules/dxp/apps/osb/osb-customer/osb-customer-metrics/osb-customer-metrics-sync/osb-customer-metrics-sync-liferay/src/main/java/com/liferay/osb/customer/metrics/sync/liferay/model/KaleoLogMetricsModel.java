/**
 * Copyright (c) 2000-present Liferay, Inc. All rights reserved.
 *
 * The contents of this file are subject to the terms of the Liferay Enterprise
 * Subscription License ("License"). You may not use this file except in
 * compliance with the License. You can obtain a copy of the License by
 * contacting Liferay, Inc. See the License for the specific language governing
 * permissions and limitations under the License, including but not limited to
 * distribution rights of the Software.
 *
 *
 *
 */

package com.liferay.osb.customer.metrics.sync.liferay.model;

import com.liferay.osb.customer.metrics.impl.model.BaseModelMetricsModel;
import com.liferay.osb.customer.metrics.model.MetricsModel;
import com.liferay.osb.customer.metrics.sync.liferay.model.util.MetricsTransformationUtil;
import com.liferay.portal.kernel.model.User;
import com.liferay.portal.kernel.service.UserLocalService;
import com.liferay.portal.workflow.kaleo.model.KaleoLog;

import java.util.Map;

import org.osgi.service.component.annotations.Component;
import org.osgi.service.component.annotations.Reference;

/**
 * @author Jenny Chen
 */
@Component(immediate = true, service = MetricsModel.class)
public class KaleoLogMetricsModel extends BaseModelMetricsModel<KaleoLog> {

	@Override
	public Map<String, Object> getAttributes(KaleoLog kaleoLog) {
		Map<String, Object> attributes =
			_metricsTransformationUtil.transformSharedAttributes(
				kaleoLog.getModelAttributes());

		String currentAssigneeClassName =
			kaleoLog.getCurrentAssigneeClassName();

		if (currentAssigneeClassName.equals(User.class.getName())) {
			long classPK = kaleoLog.getCurrentAssigneeClassPK();

			User user = _userLocalService.fetchUser(classPK);

			if (user != null) {
				attributes.put("currentAssigneeClassPK", user);
			}
		}

		String previousAssigneeClassName =
			kaleoLog.getPreviousAssigneeClassName();

		if (previousAssigneeClassName.equals(User.class.getName())) {
			long classPK = kaleoLog.getPreviousAssigneeClassPK();

			User user = _userLocalService.fetchUser(classPK);

			if (user != null) {
				attributes.put("previousAssigneeClassPK", user);
			}
		}

		return attributes;
	}

	@Override
	public Class getModelClass() {
		return KaleoLog.class;
	}

	@Reference
	private MetricsTransformationUtil _metricsTransformationUtil;

	@Reference
	private UserLocalService _userLocalService;

}