...
<liferay-util:include page="/alloy_mvc/jsp/testray/views/header.jsp" servletContext="<%= application %>">
	<liferay-util:param name="title" value="${testraySuite.name}" />
...
<portlet:actionURL var="updateTestraySuiteURL">
	<portlet:param name="controller" value="suites" />
	<portlet:param name="action" value="update" />
</portlet:actionURL>

<div class="testray-card">
	<aui:form action="${updateTestraySuiteURL}" method="post" name="editSuiteForm">
		...
		<aui:input name="redirect" type="hidden" value="${not empty param.redirect ? param.redirect : viewTestraySuitesURL}" />

		<aui:input name="id" type="hidden" value="${testraySuite.testraySuiteId}" />

		<aui:input name="addTestrayCaseIds" type="hidden" />
		<aui:input name="caseParametersString" type="hidden" value="${caseParametersString}" />
		...
		<aui:input checked="${not empty caseParametersString}" label="smart-suite" name="smartSuite" onChange="Liferay.Util.toggleDisabled(${htmlNamespace}selectCases, this.checked); Liferay.Util.toggleDisabled(${htmlNamespace}selectCaseParameters, !this.checked);" type="checkbox" />
		...
		<aui:button-row>
			...
			<aui:button disabled="${not empty caseParametersString}" name="selectCases" onClick="${selectTestrayCasesURL}" value="select-cases" />

			<aui:button disabled="${empty caseParametersString}" name="selectCaseParameters" onClick="${htmlNamespace}openSelectCaseParameters();" value="select-case-parameters" />
		</aui:button-row>
		...
		>
			...
			>
				...
				<c:if test="${empty caseParametersString}">
					<liferay-ui:search-container-column-text
						align="right"
					>
						<liferay-ui:icon
		...
		<aui:button-row>
			...
			<aui:button href="${not empty param.redirect ? param.redirect : viewTestraySuitesURL}" value="cancel" />
		</aui:button-row>
	</aui:form>
</div>
...
<aui:script>
			...
			testrayCases.forEach(
				function(testrayCase) {
					if (searchContainerData.indexOf(testrayCase.testrayCaseId.toString()) < 0) {
						var editSuiteForm = Liferay.Form.get('${htmlNamespace}editSuiteForm');

						var smartSuiteInput = editSuiteForm.form.smartSuite;
	...
	Liferay.provide(
		...
		function() {
			...
			if (editSuiteForm) {
				var caseParametersStringElement = editSuiteForm.form.caseParametersString;
