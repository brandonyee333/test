 ...
 */
public class InterestsHistogramTransformationJSONArrayFunction
	extends BaseTransformationJSONArrayFunction {

	public InterestsHistogramTransformationJSONArrayFunction(
		...
		for (InternalDateHistogram.Bucket bucket :
			...
			jsonArray.put(
				JSONUtil.put(
					...
					"scoreAvg",
					getAggregationValue(innerAggregations, "scoreAvg")
				).put(
					"totalElements", bucket.getDocCount()
				).put(
					"viewsSum",
					getAggregationValue(innerAggregations, "viewsSum")
				));
		}
