...
class GenerateItemsSparkJob(BaseSparkJob):
        ...
        pub_sub_bridge.send(
            PubSubMessage(
                "content_recommendation",
                {
                    ...
                    "body": {
                        "context": {"itemsDatasetCount": items_data_frame.count()}
                    },
                },
            )
        )
    ...
    def run(self):
        items_data_frame = (
            self.spark_session.table("user_item_interactions")
            .select(col("ITEM_ID").alias("itemId"))
            .distinct()
        )
...
class GenerateUserItemInteractionsSparkJob(BaseSparkJob):
        ...
        pub_sub_bridge.send(
            PubSubMessage(
                "content_recommendation",
                {
                    ...
                    "body": {
                        "context": {
                            "userItemInteractionsDatasetCount": user_item_interactions_data_frame.count()
                        }
                    },
                },
            )
    ...
    def run(self):
        user_item_interactions_data_frame = (
            self.spark_session.table("analytics_events")
            .withColumn(
                "event_timestamp", unix_timestamp(col("eventDate").cast("timestamp"))
            )
            .select(
...
class ReadAnalyticsEventsSparkJob(BaseSparkJob):
    ...
    def _get_analytics_events_paths(self):
        ...
        analytics_events_paths_map = map(
            ...
            file_system_instance.listStatus(jvm.org.apache.hadoop.fs.Path(bucket_path)),
        )
    ...
    def _get_analytics_events_paths_filtered(self):
        ...
        delta = datetime.utcnow() - timedelta(days=self._get_maximum_days_delta())
    ...
    def _get_expression(self, filter_string, negate):
        ...
        if tokens[1] == "=":
            ...
            expression = '(rlike(context.`{}`, "(?i){}"))'.format(tokens[0], tokens[2])
    ...
    def _get_filter_expresssions(self):
        ...
        for parameter in job_parameters:
            if parameter.get("name") == "excludeFilter":
                expressions.append(self._get_expression(parameter.get("value"), True))
            elif parameter.get("name") == "includeFilter":
                expressions.append(self._get_expression(parameter.get("value"), False))
    ...
    def run(self):
        ...
        analytics_events_data_frame = (
            data_frame_reader.json(self._get_analytics_events_paths_filtered())
            .withColumn(
                "days_delta", datediff(current_date(), expr("to_date(eventDate)"))
            )
            .withColumn(
                "interactions", count("userId").over(Window.partitionBy("userId"))
            )
        )
        ...
        analytics_events_data_frame = analytics_events_data_frame.filter(
            col("eventProperties.viewDuration") >= self._minimum_view_duration_threshold
        )
...
class ReadRecommendedItemsSparkJob(BaseSparkJob):
    def run(self):
        ...
        recommended_items_data_frame = (
            data_frame_reader.json(
            ...
            .withColumn("jobId", lit(self.spark_application.job_id))
            .selectExpr(
                "sha1(concat(jobId, input.itemId)) as id",
                "input.itemId as itemId",
                "jobId",
                "output.recommendedItems as recommendedItemIds",
            )
        )

        recommended_items_data_frame.createOrReplaceTempView("recommended_items")
...
class WriteDataframeSparkJob(BaseSparkJob):
    ...
    def run(self):
        ...
        data_frame_writer.format(self._output_format).mode("overwrite").option(
            "header", "True"
        ).save(
            "{}/{}/{}/{}".format(
                self.spark_application_configuration.get("aws.storage.path"),
...
class WriteItemsSparkJob(WriteDataframeSparkJob):
    def __init__(self, spark_application):
        super(WriteItemsSparkJob, self).__init__("items", "json", spark_application)
...
class WriteRecommendedItemsSparkJob(BaseSparkJob):
    def run(self):
        ...
        data_frame_writer.json(
            "{}/{}/{}/{}".format(
                self.spark_application_configuration.get(
                    "google.storage.path.content-recommendations"
                ),
                self.spark_application_args.lcp_project_id,
                self.spark_application.job_id,
                self.spark_application.job_run_id,
            )
        )
