...
from liferay.content_recommendation.job import (
...
class ContentRecommendationApplication(BaseSparkApplication):
    ...
    def _create_spark_conf(self):
        ...
        spark_conf.set("fs.s3a.access.key", self.configuration.get("aws.access.key.id"))
        spark_conf.set(
            "fs.s3a.secret.key", self.configuration.get("aws.secret.access.key")
        )
        spark_conf.set(
            "spark.jars.packages", self.configuration.get("spark.jars.packages")
        )
