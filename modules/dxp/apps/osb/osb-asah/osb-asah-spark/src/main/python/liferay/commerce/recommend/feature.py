...
class CommerceFeatureExtractor(object):
    ...
    def _extract_features(self, column_name):
        if column_name in self._categorical_column_names:
            ...
            self._log.warn("Unable to find feature extractor for: " + column_name)
    ...
    def count_vectorizer_encoder(column_name):
        ...
        stages = [CountVectorizer(inputCol=column_name, outputCol=feature_column_name)]
    ...
    def text_encoder(column_name):
        ...
        stages = [Tokenizer(inputCol=column_name, outputCol=column_name + "Token")]

        stages += [
            StopWordsRemover(
                inputCol=column_name + "Token", outputCol=column_name + "TokenFiltered"
            )
        ]

        stages += [
            CountVectorizer(
                inputCol=column_name + "TokenFiltered", outputCol=column_name + "Count"
            )
        ]

        stages += [IDF(inputCol=column_name + "Count", outputCol=feature_column_name)]
...
class MAPEvaluator(Evaluator):
    ...
    def _evaluate(self, data_frame):
        ...
        data_frame = data_frame.withColumn("binary", binary_expression.cast("double"))
