...
class BaseDataFrameReaderSparkJob(BaseSparkJob):
    def __init__(
        self, spark_application, file_pattern, table_name, cache=True, latest=True
    ):
    ...
    def _get_bucket_path(self):
        ...
        if self.latest:
            ...
            file_system_instance = file_system.get(
                jvm.java.net.URI(bucket_path), spark_context._jsc.hadoopConfiguration()
            )

            file_status_list = file_system_instance.globStatus(
                jvm.org.apache.hadoop.fs.Path("{}*/_SUCCESS".format(bucket_path))
            )
        ...
        self.spark_application.log.info("Loading data from: {}".format(bucket_path))
...
class BaseDataFrameWriterSparkJob(BaseSparkJob):
    ...
    def run(self):
        ...
        data_frame_writer.format("json").mode("overwrite").save(self._get_bucket_path())
...
class ContextUserInteractionRecommendationDataFrameWriterSparkJob(
    ...
    def pre_process(self, data_frame):
        ...
        data_frame = data_frame.withColumn(
            "jobId", lit(self.spark_application_configuration.get("spark.app.id"))
        )
        data_frame = data_frame.withColumn(
            "entryClassPK", col("commerceAccountId").cast("long")
        )
        data_frame = data_frame.withColumn(
            "recommendedEntryClassPK", col("CPDefinitionId").cast("long")
        )
...
class ProductContentDataFrameReaderSparkJob(BaseDataFrameReaderSparkJob):
    ...
    def _get_product_specifications_data_frame(self, products_data_frame):
        ...
        product_specifications_data_frame = products_data_frame.select(
            "id", explode("productSpecifications").alias("productSpecifications")
        )
        ...
        for product_specification_id in product_specification_ids:
            specification_column_name = "SPECIFICATION_" + str(product_specification_id)
        ...
        product_specification_data_frame = products_data_frame.select("id").distinct()
...
class ProductContentPipelineSparkJob(BaseSparkJob):
    ...
    def run(self):
        ...
        feature_column_names = [
            column
            for column in products_data_frame.columns
            if column not in self._commerce_exclude_cp_columns
        ]

        products_data_frame = (
            products_data_frame.withColumn(
                "description", coalesce(col("description"), col("name"))
            )
            .withColumn("description", regexp_replace("description", r"[^\w\s]", ""))
...
class ProductContentRecommendationSparkJob(BaseSparkJob):
    def __init__(self, spark_application):
        super(ProductContentRecommendationSparkJob, self).__init__(spark_application)
    ...
    def run(self):
        ...
        window = Window.partitionBy(col("entryClassPK")).orderBy(col("score").desc())
        ...
        recommendations_data_frame = recommendations_data_frame.withColumn(
            "recommendedEntryClassPK", col("entryClassPK2").cast("long")
        )

        recommendations_data_frame = (
            recommendations_data_frame.drop("entryClassPK2")
            .drop("features")
            .drop("features2")
        )
...
class ProductInteractionDataFrameReaderSparkJob(BaseDataFrameReaderSparkJob):
    ...
    def _post_process(self, data_frame):
        return data_frame.selectExpr(
            ...
            "categories.id AS assetCategoryIds",
...
class ProductInteractionRecommendationSparkJob(BaseSparkJob):
    ...
    def run(self):
        ...
        recommendations_data_frame = item_factors_data_frame.crossJoin(
            item_factors_data_frame.selectExpr("id as id2", "features as features2")
        )

        recommendations_data_frame = recommendations_data_frame.selectExpr(
            "id",
            "toDenseVector(features) as features",
            "id2",
        ...
        recommendations_data_frame = recommendations_data_frame.drop("features2")
        ...
        self.spark_session.catalog.cacheTable("product_interaction_recommendations")
...
class UserInteractionCollaborativeFilteringSparkJob(BaseSparkJob):
    ...
    def run(self):
        user_item_rating_data_frame = self.spark_session.table("user_item_rating_table")
        ...
        self._log_model_performance(best_model, test_data_frame, train_data_frame)

        requested_catalog_coverage = self._get_requested_catalog_coverage()

        self._log.info(
            "Generating recommendations of {} items per user".format(
        ...
        self.spark_session.catalog.cacheTable("context_user_interaction_recommendation")
    ...
    def _get_context_recommendations_data_frame(self, recommendations_data_frame):
        product_interaction_data_frame = self.spark_session.table(
            "product_interactions"
        ).select("CPDefinitionId", "assetCategoryIds")

        return (
            recommendations_data_frame.selectExpr(
                "commerceAccountId",
    ...
    def _log_model_performance(self, model, test_data_frame, train_data_frame):
        ...
        test_map = self._get_evaluator().evaluate(model.transform(test_data_frame))

        self._log.info("Model performance on TEST set: {}".format(test_map))

        train_map = self._get_evaluator().evaluate(model.transform(train_data_frame))
...
class UserInteractionDataPreparationSparkJob(BaseSparkJob):
    ...
    def run(self):
        ...
        order_interaction_data_frame = self.spark_session.table("order_interactions")
        ...
        user_item_ratings_data_frame.createOrReplaceTempView("user_item_rating_table")
...
class FrequentPatternOrderDataFrameReaderSparkJob(BaseDataFrameReaderSparkJob):
    ...
    def _post_process(self, data_frame):
        return data_frame.selectExpr("id as orderId", "EXPLODE(orderItems.sku) as sku")


class FrequentPatternProductDataFrameReaderSparkJob(BaseDataFrameReaderSparkJob):
    def __init__(self, spark_application):
        super().__init__(
            spark_application,
            "com.liferay.headless.commerce.admin.catalog.dto.v1_0.Product",
    ...
    def _post_process(self, data_frame):
        return data_frame.selectExpr("id AS CPDefinitionId", "EXPLODE(skus.sku) AS sku")
...
class FrequentPatternRecommendationSparkJob(BaseSparkJob):
    ...
    def run(self):
        ...
        fp_growth = FPGrowth(minSupport=min_support, minConfidence=min_confidence)
...
class FrequentPatternPostProcessRecommendationSparkJob(BaseSparkJob):
    ...
    def run(self):
        association_rules_data_frame = self.spark_session.table("association_rules")

        frequent_itemsets_data_frame = self.spark_session.table("frequent_itemsets")

        order_data_frame = self.spark_session.table("order")

        order_count = order_data_frame.count()

        association_rules_data_frame = association_rules_data_frame.filter("lift > 1.0")

        association_rules_data_frame = association_rules_data_frame.join(
            frequent_itemsets_data_frame.withColumnRenamed("items", "antecedent"),
            on="antecedent",
        )
        ...
        association_rules_data_frame = association_rules_data_frame.withColumn(
            "antecedent_support", expr("antecedent_freq / {}".format(order_count))
        )
        ...
        association_rules_data_frame = association_rules_data_frame.withColumn(
            "delta", expr("1 / GREATEST(antecedent_support, consequent_support)")
        )
        ...
        association_rules_data_frame = association_rules_data_frame.select(
            "antecedent",
            explode("consequent").alias("consequent"),
            "confidence",
            "lift",
            "stdLift",
        )
        ...
        window_spec = window_spec.orderBy(col("stdLift").desc(), col("lift").desc())
...
class FrequentPatternRecommendationDataFrameWriterSparkJob(BaseDataFrameWriterSparkJob):
    def __init__(self, spark_application):
        super().__init__(
            spark_application,
            "com.liferay.headless.commerce.machine.learning.dto.v1_0."
