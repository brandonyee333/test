...
const _itemsToValue = (items: MultiselectPicklistItem[], i18n = false) =>
	items.reduce(
		(
			valueAsString: string,
			{key, label}: MultiselectPicklistItem
		): string => {
	...
	const fetchSourceItems = useCallback(() => {
		fetch(composeListTypeEntriesEndpoint(listTypeDefinitionId))
			...
			.then(({items: listTypeEntries}) => {
				const languageId = Liferay.ThemeDisplay.getLanguageId().replace(
					'_',
					'-'
				);
