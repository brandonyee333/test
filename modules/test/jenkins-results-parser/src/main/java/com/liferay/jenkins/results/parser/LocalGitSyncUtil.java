...
public class LocalGitSyncUtil {
	...
	protected static void cacheBranches(
		...
		final RemoteGitBranch upstreamRemoteGitBranch =
			gitWorkingDirectory.getRemoteGitBranch(
				...
				gitWorkingDirectory.getUpstreamRemote(), true);
	...
	protected static void deleteCacheRemoteGitBranch(
		...
		List<BaseGitBranch> cacheRemoteGitBranches = new ArrayList<>(2);
		...
		if (!cacheRemoteGitBranches.isEmpty()) {
			gitWorkingDirectory.deleteGitBranches(cacheRemoteGitBranches);
		}
	}
	...
	protected static void deleteExpiredCacheBranches(
		...
		List<BaseGitBranch> expiredBranches = new ArrayList<>();
			...
			if (lastBlock.matches("\\d+")) {
				...
				if (branchAge > _BRANCH_EXPIRE_AGE_MILLIS) {
					...
					if (repositoryBaseRemoteGitBranch != null) {
						expiredBranches.add(repositoryBaseRemoteGitBranch);
					}

					expiredBranches.add(remoteGitBranch);
		...
		gitWorkingDirectory.deleteGitBranches(expiredBranches);
	...
	protected static String synchronizeToLocalGit(
		...
		LocalGitBranch originalLocalGitBranch =
			gitWorkingDirectory.getCurrentLocalGitBranch();

		if (originalLocalGitBranch == null) {
			gitWorkingDirectory.reset("--hard");

			originalLocalGitBranch = gitWorkingDirectory.getLocalGitBranch(
				gitWorkingDirectory.getUpstreamBranchName());

			if (originalLocalGitBranch != null) {
				gitWorkingDirectory.checkoutLocalGitBranch(
					originalLocalGitBranch);
			}
		}
		...
		System.out.println(
			JenkinsResultsParserUtil.combine(
				...
				"branch is ", originalLocalGitBranch.getName(), "."));
		...
		try {
			...
			tempLocalGitBranch = gitWorkingDirectory.createLocalGitBranch(
				"temp-" + System.currentTimeMillis(), true,
				originalLocalGitBranch.getSHA());
			...
			try {
				...
				if (isBranchCached(
					...
					gitWorkingDirectory.fetch(null, cacheRemoteGitBranch);
				...
				if (gitWorkingDirectory.localGitBranchExists(
						originalLocalGitBranch.getName())) {
	...
	protected static void updateCacheBranchTimestamp(
		...
		for (RemoteGitBranch cacheRemoteGitBranch : cacheRemoteGitBranches) {
			String cacheRemoteBranchName = cacheRemoteGitBranch.getName();

			Matcher matcher = _cacheBranchPattern.matcher(
				cacheRemoteBranchName);

			if (!cacheRemoteBranchName.contains(cacheBranchName) ||
				!matcher.matches()) {
			...
			if (updated) {
				pushToAllRemotes(
					true, gitWorkingDirectory, null, cacheRemoteBranchName,
					localGitRemotes);
			...
			System.out.println(
				JenkinsResultsParserUtil.combine(
					...
					cacheRemoteBranchName, "to ", newTimestampBranchName));
	...
	protected static LocalGitBranch updateUpstreamLocalGitBranch(
		...
		gitWorkingDirectory.fetch(null, upstreamRemoteGitBranch);
	...
	private static final long _BRANCH_UPDATED_AGE_MILLIS = 1000 * 60 * 60 * 24;
