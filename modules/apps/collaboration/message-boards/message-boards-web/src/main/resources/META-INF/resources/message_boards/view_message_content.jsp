	...
	<div class="thread-controls">
		<div class="thread-actions">
			<liferay-ui:icon-list>
				...
				<c:if test="<%= !thread.isLocked() && MBMessagePermission.contains(permissionChecker, rootMessage, ActionKeys.PERMISSIONS) %>">
				...
				<c:if test="<%= enableRSS && MBMessagePermission.contains(permissionChecker, rootMessage, ActionKeys.VIEW) %>">
					<liferay-ui:rss
						delta="<%= rssDelta %>"
						displayStyle="<%= rssDisplayStyle %>"
						feedType="<%= rssFeedType %>"
						url="<%= MBUtil.getRSSURL(plid, 0, rootMessage.getThreadId(), 0, themeDisplay) %>"
					/>
				</c:if>

				<c:if test="<%= MBMessagePermission.contains(permissionChecker, rootMessage, ActionKeys.SUBSCRIBE) && (mbGroupServiceSettings.isEmailMessageAddedEnabled() || mbGroupServiceSettings.isEmailMessageUpdatedEnabled()) %>">
					<c:choose>
