...
public class PayPalCommercePaymentIntegration
	...
	public CommercePaymentEntry authorize(
		...
		try {
			...
			ordersAuthorizeRequest.prefer("return=representation");
			...
			if (authorizeHttpResponse.statusCode() == 201) {
				...
				success = true;
				transactionId = authorizeOrder.purchaseUnits(
				).get(
					0
				).payments(
				).authorizations(
	...
	public CommercePaymentEntry setUpPayment(
		...
		try {
			...
			ordersCreateRequest.payPalPartnerAttributionId(
				"Liferay_SP_PPCP_API");
