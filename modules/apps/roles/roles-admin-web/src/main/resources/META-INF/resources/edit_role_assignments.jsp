...
<c:if test='<%= !SegmentsEntryDisplayContext.isRoleSegmentationEnabled(themeDisplay.getCompanyId()) && tabs2.equals("segments") %>'>
	...
	>
		...
		<span><%= LanguageUtil.get(request, "to-enable,-go-to") %></span>
		...
		<c:choose>
			<c:when test="<%= segmentsConfigurationURL != null %>">
				<clay:link
					...
					label='<%= LanguageUtil.get(request, "instance-settings") %>'
				/>
			</c:when>
			<c:otherwise>
				<span><%= LanguageUtil.get(request, "instance-settings") %></span>
			</c:otherwise>
		</c:choose>
	</clay:stripe>
</c:if>
