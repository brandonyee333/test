				...
				<c:otherwise>
					...
					%>
						...
						<li class="<%= ((cur - 3) > 1) ? "" : "page-item" %>">
							<a class="<%= ((cur - 3) > 1) ? "dropdown-icon" : "dropdown-icon page-link" %>" href="<%= _getHREF(formName, namespace + curParam, i, jsCall, url, urlAnchor) %>" onclick="<%= forcePost ? _getOnClick(namespace, curParam, i) : "" %>"><span class="sr-only"><liferay-ui:message key="page" /></span><%= i %></a>
						</li>
